...
def purchase(List):
    ...
    q = {}  # assign empty dictionary with variable name 'q'. #this dictionary is going to store what the customer bought.
    flag = "Y" #this is to make sure that the customer still wants to purchase items.
    while flag.upper() == "Y":  # check and go if flag is 'Y' or 'y'.
        product = input("\nWhat product do you want to buy? ")
        product_name = product.upper()  # changes the string value to upper case.

        if product_name == L[0][0].upper() or product_name == L[1][0].upper() or product_name == L[2][0].upper():
            p = True
            while p == True: #to make sure that the user entered a product available.
                    p_quantity = input("How many " + product + " do you want to buy: ")
                    check= p_quantity.isnumeric()
                    if check== True:
                        p=False
                    else:
                        print("\t\tError!!!\nPlease enter integer value!! ")
                    
            if product_name == L[0][0].upper() and int(p_quantity) <= int(L[0][2]): #if product is phone and the quantity entered by customer is less than the available 
                q[product_name] = p_quantity #in this case, it adds to the dictionary q- "PHONE":"quantity the customer purchased'
            elif product_name == L[1][0].upper() and int(p_quantity) <= int(L[1][2]):
                q[product_name] = p_quantity
            elif product_name == L[2][0].upper() and int(p_quantity) <= int(L[2][2]):
                q[product_name] = p_quantity
            else:
                print(
                ...
                #prints if the quantity entered by the customer is ore than the stock
            flag = (input(a_name + " do you want buy more products?(Y/N)")) #if the answer is yes it executes that part again 
        else: #if the user enters a product not available 
    ...
    for keys in q.keys():
        if keys == L[0][0].upper():  # executes this operation if product is phone entered by customer.
            ...
            f_amount += (p_price * p_num) #calculates the total price
    ...
    print("\nYour discountable total amount is: ", f_amount) #results the final amount to be paid by the customer 
    ...
    dis = 0.0
    if (f_amount >= 5000) and (f_amount < 10000): #if the total amount is in between 5000 and 10000
        discount = disc
        if discount <= 5.0: #gives discount only if the expected dictount entered by the user is less than 5%
            ...
            print("You did not got your expected " + str(
                ...
                disc) + "% discount.")
            print("You got basic 5% discount and discounted amount is:", dis) #if the expected discount amount entered by user is more than 5%, it automatically gives 5% discount
    elif f_amount >= 10000:
        discount = disc
        if discount <= 10.0:
            dis = (discount * f_amount) / 100
